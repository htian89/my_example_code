			    ...
			    const char *buf, size_t count)
{
	...
	if (sockfd != -1) {
		...
		spin_unlock(&sdev->ud.lock);
		usbip_dbg_crash("kthread_get_run:stub_rx_loop\n");
		sdev->ud.tcp_rx = kthread_get_run(stub_rx_loop, &sdev->ud, "stub_rx");
		usbip_dbg_crash("kthread_get_run:stub_tx_loop\n");
...
static int send_disconnect(struct stub_device *sdev)
{
	...
	int busnum;
	int devnum;
	busnum = cpu_to_be32(sdev->devid>>16);
	devnum = cpu_to_be32(sdev->devid&0xffff);
	SET_UINT16(msg_buff, 0, (uint16)type);
	SET_UINT16(msg_buff, 2, (uint16)flag);
	SET_UINT32(msg_buff, 4, (uint32)length);
	memcpy(msg_buff+8,&busnum,4);
	memcpy(msg_buff+12,&devnum,4);
	ret = usbip_snd(sdev->ud.tcp_socket,(void *)msg_buff,16);
	if(ret!=16)
	{
		dev_err(dev, "send disconnect err\n");
		usbip_dbg_crash("send disconnect err\n");
		return 0;
	}
	usbip_dbg_temp("send disconnect package to server, ret = %d\n",ret);
	return ret;
}
...
static void stub_shutdown_connection(struct usbip_device *ud)
{
	...
	/* 1. stop threads */
	/*
	if (ud->tcp_rx && (ud->tcp_rx)->exit_state == 0 )
	{
		kthread_stop_put(ud->tcp_rx);
        usbip_dbg_temp("kthread_stop tcp_rx\n");
	}
	if (ud->tcp_tx && (ud->tcp_tx)->exit_state == 0 )
	{
		kthread_stop_put(ud->tcp_tx);
		usbip_dbg_temp("kthread_stop tcp_tx\n");
	}*/
	if (ud->tcp_rx)
	{
		kthread_stop_put(ud->tcp_rx);
	...
	usbip_dbg_temp("stub_shutdown_connection over\n");
}
...
static void stub_device_reset(struct usbip_device *ud)
{
    ...
    /* This function always failed */
//	ret = usb_lock_device_for_reset(udev, sdev->interface);
//	if (ret < 0) {
//		dev_err(&udev->dev, "lock for reset, ret = %d\n", ret);
//		spin_lock(&ud->lock);
...
//	usb_unlock_device(udev);
	...
	spin_lock(&ud->lock);
	if (ret) {
		dev_err(&udev->dev, "device reset, ret = %d\n", ret);
		ud->status = SDEV_ST_ERROR;
	} else {
		dev_info(&udev->dev, "device reset\n");
		usbip_dbg_temp("device reset, ret = %d\n", ret);
		      ...
		      const struct usb_device_id *id)
{
	...
	if (!busid_priv || (busid_priv->status == STUB_BUSID_REMOV) ||
		...
		dev_info(&interface->dev, "%s is not in match_busid table... "
			 "skip!\n", udev_busid);
		usbip_dbg_crash("%s is not in match_busid table... "
			 "skip!\n", udev_busid);
	...
	if (busid_priv->status == STUB_BUSID_ALLOC) {
		...
		dev_info(&interface->dev, "usbip: register new interface "
			 "(bus %u dev %u ifn %u)\n",
	...
	usbip_dbg_crash("stub_device_alloc\n");
...
static void shutdown_busid(struct bus_id_priv *busid_priv)
{
	if (busid_priv->sdev && !busid_priv->shutdown_busid) {
		...
		usbip_dbg_temp("kthread_stop eh\n");
	}
}
...
//	send_disconnect(sdev);
	...
	dev_info(&interface->dev, "usb set intfdata NULL\n");
	usb_set_intfdata(interface, NULL);
