...
static int get_pipe(struct stub_device *sdev, int epnum, int dir)
{
	...
	if (usb_endpoint_xfer_control(epd)) {
		usbip_dbg_header("is control endpoint %d\n",epnum);
	...
	if (usb_endpoint_xfer_bulk(epd)) {
		usbip_dbg_header("is bulk endpoint %d\n",epnum);
		if (dir == USBIP_DIR_OUT)
	...
	if (usb_endpoint_xfer_int(epd)) {
		usbip_dbg_header("is int endpoint %d\n",epnum);
		if (dir == USBIP_DIR_OUT)
	...
	if (usb_endpoint_xfer_isoc(epd)) {
		usbip_dbg_header("is isoc endpoint %d\n",epnum);
		if (dir == USBIP_DIR_OUT)
...
static void stub_rx_pdu(struct usbip_device *ud)
{
	...
	buff = kzalloc(56,GFP_KERNEL);
	
	/* 1. receive a pdu header */
	ret = usbip_recv(ud->tcp_socket, (char *) buff, 56);
	if (ret != 56) {
