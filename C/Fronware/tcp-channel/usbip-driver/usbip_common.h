...
enum {
	...
	usbip_debug_stub_cmp	= (1 << 4),
	usbip_debug_stub_dev	= (1 << 5),
	usbip_debug_stub_rx	= (1 << 6),
	usbip_debug_stub_tx	= (1 << 7),
	...
	usbip_debug_temp         = (1 << 13 ),
	usbip_debug_header       = (1 << 14 ),
	usbip_debug_data         = (1 << 15 ),
	
	usbip_debug_crash        = (1 << 16 ),
};
...
#define usbip_dbg_flag_temp   (usbip_debug_flag & usbip_debug_temp)
#define usbip_dbg_flag_header   (usbip_debug_flag & usbip_debug_header)
#define usbip_dbg_flag_data   (usbip_debug_flag & usbip_debug_data)
#define usbip_dbg_flag_crash  (usbip_debug_flag & usbip_debug_crash)
...
extern char printkbuff[128];

#define usbip_dbg_to_file(fmt, args...) \
	do {\
		snprintf(printkbuff, 128, fmt, ##args);\
		pfile->f_op->write(pfile, (char *)printkbuff, strlen(printkbuff), &pfile->f_pos);\
	} while(0)

#define usbip_dbg_with_flag(flag, fmt, args...)		\
	do {						\
		if (flag & usbip_debug_flag)	{	\
			usbip_dbg_to_file(fmt, ##args);		}\
	} while (0)

#define usbip_dbg_with_header(flag, fmt, args...)		\
	do {						\
		if (flag & usbip_debug_flag)	{	\
			usbip_dbg_to_file(fmt,##args);}	\
	} while (0)
...
#define usbip_dbg_stub_dev(fmt, args...) \
	usbip_dbg_with_flag(usbip_debug_stub_dev, fmt , ##args)
...
#define usbip_dbg_temp(fmt, args...)		\
	usbip_dbg_with_flag(usbip_debug_temp, fmt , ##args)
#define usbip_dbg_header(fmt, args...)		\
	usbip_dbg_with_header(usbip_debug_header, fmt , ##args)
#define usbip_dbg_data(fmt, args...)		\
	usbip_dbg_with_header(usbip_debug_data, fmt , ##args)
	
#define usbip_dbg_crash(fmt, args...)		\
	usbip_dbg_with_header(usbip_debug_crash, fmt , ##args)
