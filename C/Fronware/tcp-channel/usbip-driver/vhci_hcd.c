...
static int vhci_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	vdev = priv->vdev;
 	if(!vdev->seqnum)
	{
		printk("cann't unlink\n");
	}

	if (!vdev->ud.tcp_socket || !vdev->seqnum) {
...
static void vhci_shutdown_connection(struct usbip_device *ud)
{
	...
	if (vdev->ud.tcp_rx)
	{
		...
		vdev->ud.tcp_rx = NULL;
		usbip_dbg_temp("kthread stop vhci rx\n");
	}
	if (vdev->ud.tcp_tx)
	{
		kthread_stop_put(vdev->ud.tcp_tx);
		vdev->ud.tcp_tx = NULL;
		usbip_dbg_temp("kthread stop vhci tx\n");
	}
